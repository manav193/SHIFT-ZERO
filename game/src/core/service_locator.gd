...
func register(interface_name: String, instance: Object) -> void:
    if _sealed:
        Logger.error("ServiceLocator", "attempted to register '%s' after seal()" % interface_name)
        return
    if _services.has(interface_name):
        Logger.warn("ServiceLocator", "overriding registered '%s'" % interface_name)
    _services[interface_name] = instance
    Logger.debug("ServiceLocator", "registered %s -> %s" % [interface_name, instance.get_class()])


func get_service(interface_name: String) -> Object:
    if not _services.has(interface_name):
        Logger.error("ServiceLocator", "no service registered for '%s'" % interface_name)
        return null
...
func seal() -> void:
    _sealed = true
    Logger.info("ServiceLocator", "sealed with %d services" % _services.size())
