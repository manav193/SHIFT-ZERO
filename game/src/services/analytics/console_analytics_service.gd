...
##
## Default M0 implementation — emits structured JSON lines to the Logger.
...
func init(consent: Dictionary) -> void:
    ...
    Logger.info("Analytics", "console analytics ready. consent=%s" % consent)
...
func set_consent(consent: Dictionary) -> void:
    _consent = consent.duplicate()
    Logger.info("Analytics", "consent updated: %s" % consent)
