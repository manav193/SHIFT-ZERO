...
func _ready() -> void:
    ...
    print("Modifier", "manager ready. pool=%d dur=%.1fs gap=[%.1f,%.1f]s first=%.1fs" % [
...
func _activate_random() -> void:
    ...
    print("Modifier", "activated id=%s duration=%.1fs" % [mod.id(), _duration_s])
...
func _expire() -> void:
    ...
    print("Modifier", "expired id=%s" % id)
    _schedule_next()
